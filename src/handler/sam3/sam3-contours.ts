...
function packedMaskSize(mask: Segmentation): { width: number; height: number } {
...
export function maskToPolygons(mask: Segmentation, options: Sam3MaskPolygonOptions): number[][][] {
...
export function maskToPolygon(mask: Segmentation, options: Sam3MaskPolygonOptions): number[][] {
  return maskToPolygons(mask, options)[0] ?? [];
}

function tracePackedMaskContours(mask: Segmentation): number[][][] {
